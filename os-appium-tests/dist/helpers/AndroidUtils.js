...
function getElemByText(text, throwError = true, waitTime = DEFAULT_TIMEOUT) {
...
function getElemByPartialId(id, throwError = true, waitTime = DEFAULT_TIMEOUT) {
    let sel = 'android=' + getByPartialIdSelector(id);
    return Context_1.waitForElement(sel, throwError, waitTime);
}
exports.getElemByPartialId = getElemByPartialId;
function getElemById(id, throwError = true, waitTime = DEFAULT_TIMEOUT) {
    let sel = 'android=' + getByIdSelector(id);
    return Context_1.waitForElement(sel, throwError, waitTime);
}
exports.getElemById = getElemById;
function getElemByClass(className, throwError = true, waitTime = DEFAULT_TIMEOUT) {
    let sel = 'android=' + getByClassSelector(className);
    return Context_1.waitForElement(sel, throwError, waitTime);
}
exports.getElemByClass = getElemByClass;
function getElemInScroll(selector, throwError = true, waitTime = DEFAULT_TIMEOUT) {
    const sel = 'android=' + getElemInScrollSelector(selector);
    return Context_1.waitForElement(sel, throwError, waitTime);
}
exports.getElemInScroll = getElemInScroll;
function getPermissionAlertAllow(throwError = true, waitTime = DEFAULT_TIMEOUT) {
...
function getPermissionAlertDeny(throwError = true, waitTime = DEFAULT_TIMEOUT) {
    //TODO Review if selector works for other android versions
